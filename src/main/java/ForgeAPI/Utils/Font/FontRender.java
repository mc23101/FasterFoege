...
public class FontRender {
    private static final TextureLoader[] UNICODE_PAGE_LOCATIONS = new TextureLoader[256];
    protected final int[] charWidth = new int[256];
    public int FONT_HEIGHT = 4;
    public Random fontRandom = new Random();
    protected final byte[] glyphWidth = new byte[65536];
    private final int[] colorCode = new int[32];
    protected final TextureLoader locationFontTexture;
    ...
    private List<TextureLoader> fontTextureId;

    public FontRender(GameSettings gameSettingsIn, TextureLoader textureLoader, boolean unicode)
    {
        this.locationFontTexture = textureLoader;
        this.unicodeFlag = unicode;
        textureLoader.bindTexture();
//        fontTextureId.add(textureLoader);
    ...
    {
        BufferedImage bufferedimage=locationFontTexture.getImageBuffer();
    ...
    {
        try {
            new FileInputStream(new File(ResourcesUtil.getResourcesPath("assets/texture/Font/6/font/glyph_sizes.bin"))).read(this.glyphWidth);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    protected float renderDefaultChar(int ch, boolean italic)
    {
        ...
        int j = ch / 16 * 8;
        int k = italic ? 1 : 0;
        locationFontTexture.bindTexture();
        int l = this.charWidth[ch];
        float f = (float)l - 0.01F;
        GlStateManager.glBegin(5);
        GlStateManager.glTexCoord2f((float)i /576.0F, (float)j / 576.0F);
        GlStateManager.glVertex3f(this.posX + (float)k, this.posY, 0.0F);
        GlStateManager.glTexCoord2f((float)i / 576.0F, ((float)j + 7.99F) / 576.0F);
        GlStateManager.glVertex3f(this.posX - (float)k, this.posY + 7.99F, 0.0F);
        GlStateManager.glTexCoord2f(((float)i + f - 1.0F) / 576.0F, (float)j / 576.0F);
        GlStateManager.glVertex3f(this.posX + f - 1.0F + (float)k, this.posY, 0.0F);
        GlStateManager.glTexCoord2f(((float)i + f - 1.0F) / 576.0F, ((float)j + 7.99F) / 576.0F);
     ...
     * 获取Unicode页面位置
     * */
    private TextureLoader getUnicodePageLocation(int page)
    {
        if (UNICODE_PAGE_LOCATIONS[page] == null)
        {
            String path=ResourcesUtil.getResourcesPath(String.format("assets/texture/Font/6/textures/font/unicode_page_%02x.png", page));
            UNICODE_PAGE_LOCATIONS[page] = new TextureLoader(new File(path));
        }
    ...
    {
        this.getUnicodePageLocation(page).bindTexture();
    }
    ...
    protected float renderUnicodeChar(char ch, boolean italic)
    {
        ...
        else
        {
            ...
            GlStateManager.glBegin(5);
            GlStateManager.glTexCoord2f(f2 / 576.0F, f3 / 576.0F);
            GlStateManager.glVertex3f(this.posX + f5, this.posY, 0.0F);
            GlStateManager.glTexCoord2f(f2 /576.0F, (f3 + 15.98F) / 576.0F);
            GlStateManager.glVertex3f(this.posX - f5, this.posY + 7.99F, 0.0F);
            GlStateManager.glTexCoord2f((f2 + f4) / 576.0F, f3 / 576.0F);
            GlStateManager.glVertex3f(this.posX + f4 / 2.0F + f5, this.posY, 0.0F);
            GlStateManager.glTexCoord2f((f2 + f4) / 576.0F, (f3 + 15.98F) / 576.0F);
        ...
        {
            {
                ...
                {
                    System.out.println("drwastrikethroughStyle");
