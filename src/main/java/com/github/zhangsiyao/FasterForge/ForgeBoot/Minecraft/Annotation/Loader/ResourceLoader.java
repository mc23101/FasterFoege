...
import com.github.zhangsiyao.FasterForge.ForgeBoot.Minecraft.Annotation.MinecraftResource;
...
import com.github.zhangsiyao.FasterForge.Resources.BaseResource;
...
public class ResourceLoader {
            ...
            for(Field field:fieldsAnnotatedWith){
                ...
                if(canRegister){
                    ...
                    if(resourceObject!=null){
                        if(isFile){
                            resource=new BaseResource(location,(File) resourceObject);
                        }else if(isString){
                            resource=ResourceLoader.of(location,(String) resourceObject);
                        }else if(isURL){
                            resource=new BaseResource(location,(URL)resourceObject);
                        }else if(isIResource){
    ...
    private static IResource of(ResourceLocation location,String path){
        ...
        if(file.exists()&&file.isFile()){
            return new BaseResource(location,file);
        }else{
            try {
                URL url=new URL(path);
                return new BaseResource(location,url);
