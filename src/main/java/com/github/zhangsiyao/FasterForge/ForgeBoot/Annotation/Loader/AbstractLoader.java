...
public abstract class AbstractLoader implements IAnnotationLoader{
    ...
    public boolean checkMethod(Method method,Class parent,Class annotation){
        if(!Modifier.isStatic(method.getModifiers())){
            logger.error("=================================================================================");
            logger.error("注解："+annotation.getSimpleName()+"使用错误");
            logger.error("失败原因："+annotation.getSimpleName()+"应作用在static方法上");
            logger.error("错误位置:"+method.getDeclaringClass().getName()+"类中的"+method.getName()+"方法");
            logger.error("=================================================================================");
            return false;
        }
        ...
        if(child.getName().equals(Object.class.getName())){
            logger.error("=================================================================================");
            logger.error("注解："+annotation.getSimpleName()+"使用错误");
            logger.error("失败原因："+method.getName()+"方法返回的类型应为"+parent.getName()+"的子类");
            logger.error("错误位置:"+method.getDeclaringClass().getName()+"类中的"+method.getName()+"方法");
            logger.error("=================================================================================");
            return false;
        }else {
